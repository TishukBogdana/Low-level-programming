...
map = (mem_t*) mmap( (void*) (x+mem-x%mem), size, PROT_READ|PROT_WRITE, MAP_FIXED|MAP_ANONYMOUS|MAP_SHARED,-1,0);
...
void* m_calloc(size_t mul, size_t size){
 return m_malloc(mul*size);
}

void* m_realloc(void* ptr, size_t new_size){
int i=0;
size_t pointer = (size_t)ptr-sizeof(mem_t);
unsigned char* ptr_new =(unsigned char*)m_malloc(new_size);
mem_t* ptr_mem = (mem_t*)pointer;
size_t cap = ptr_mem->capacity;
for(i=0; i<cap;i++){
*(ptr_new+i)= *((unsigned char*)ptr+i);
}
m_free(ptr);
return (void*) ptr_new;
}
